...
\section{Realization}
The project was realized as follows: \mytitle{} is built as a common code base between Undertow and Tomcat. It should be compiled in standalone and optionally loaded at run time if the user need to use the OpenSSL engine.
\todo[inline]{What I added sounds partly redundant with next paragraph}
...
\todo[inline]{Is this really an issue in the end?}
...
Currently there are 3 ways to use SSL/TLS in Tomcat, as shown on Figure \ref{fig:current}. The first one uses the OpenSSL through the APR connector (blue path). Here the APR handle encryption and networking. It provides its own socket API and transparently encrypts and decrypts data flowing through it. Since the APR is programmed in C, Tomcat-Native offers JNIs to be used in Tomcat.
...
Figure \ref{fig:goal} describes the architecture of \mytitle{} after our changes. We got rid of the APR connector as well as usage of the APR in JNIs. Most of the Java code concerning the OpenSSL engine was moved from Tomcat to \mytitle{}, so Undertow can use it. However Tomcat and Undertow use a different system to provide and instantiate the OpenSSL engine, so we kept slightly different versions of some classes in both projects.
...
Fortunately, this concerned much fewer code than we imagined.
