...
\item Abstract Tomcat Native to be used in Tomcat and Undertow through JNI calls
...
\item Extend the OpenSSL support by adding JNI calls for more functions
\end{enumerate}
...
At the moment, only Tomcat has access to OpenSSL through its library Tomcat Native. Unfortunately Undertow has to rely on the JSSE implementation to provide SSL/TLS. This leads to bad performance in Undertow.
...
Our project should streamline Tomcat Native so that it can be used in both Undertow and Tomcat, giving access to a performant implementation of TLS/SSL to Undertow while making it easier to maintain the source code for  Tomcat as well.
...
The full work plan and its associated logbook is available on the documentation repository referred in chapter \ref{sec:org}.
...
In this second task, we want to understand more precisely what is needed for the  OPENSSL JSSE % TODO TOMCAT OPENSSL JSSEENGINE   WHY j'ai pas compris dans le work

We then will write JNIs for OpenSSL to replace the ones from APR and completely remove APR from the Tomcat Native code base. as APR isn't needed anymore and Tomcat Native gives access to most of the APR functions which we don't need as we want to focus solely on OpenSSL.  This deadline should be done by the 10th April 2016.

\subsection{Task 3}
\label{subsec:task3}

At the end of the third task, we aim at having a fully working \mytitle{} working for both Tomcat and Undertow. To achieve this goal we need to understand thoroughly how the JSSE SSL Engine and Java sockets are used in both web servers as well as understand how the JSSE OpenSSL engine works in Tomcat and how could we modify it to make it work in Undertow.

\subsection{Task 4}
\label{subsec:task4}

Finally, we will have to Benchmark \mytitle{} with Red Hat to see how it behaves on different platforms. Then, if time permits, we will implement more features in \mytitle{} regarding OpenSSL. For example, implementing support the OpenSSL handshake callback would be really useful as right now we have to rely on a hack to know when the handshake is completed.

Implementing support for proprietary large frames would also be a boon as we could support more systems.

Finally, implementing different security options for OpenSSL would also be a possibility.

%TODO add table from workplan
